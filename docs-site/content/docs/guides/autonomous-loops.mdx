...
alc loop deliver --once --reset     # reset the state, then run one cycle
```

| `alc loop` flag | Effect |
|---|---|
...
Retiring archives into `loops/retired/`. It never deletes, and the member stays
