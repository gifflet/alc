...
It **validates and writes only, and never executes anything**. `alc tick` or `alc cycle` drains what lands, on its own turn. That separation is deliberate: an external caller can create demand, but it cannot make the control plane skip a step.
