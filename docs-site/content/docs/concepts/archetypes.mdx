...
| **Grower** | Iterates a built product to improve product-market fit | *Partial*: a do-it-yourself issue and error-sweep Specialist (`listen`). Real signal intake and a `regression` replenish kind are separate control-plane features, available to any Blueprint |
