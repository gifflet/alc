...
| Flag | Default | Effect |
|---|---|---|
| `--kind flow\|specialist\|run` | `flow` | Unit kind to dispatch — `run` queues a bare Blueprint as a one-stage flow |
...
| Flag | Default | Effect |
|---|---|---|
| `--once` | off | Run one cycle and exit instead of repeating |
| `--engine NAME` | manifest | Override the engine |
| `--concurrency N` | `0` | With `--once`, override the loop's drain concurrency (0 = use the definition) |
| `--status` | off | Print the loop state without running anything |
| `--reset` | off | Reset the loop state first |
| `--json` | off | With `--status`, print the state as JSON |
| `--interval S` | `300` | Seconds to sleep between cycles when repeating (0 = no sleep) |
| `--allow-dirty` | off | Silence the dirty working-tree notice |
