...
_GITIGNORE = """\
# ALC runtime state — regenerated by runs, never hand-edited or committed. The
# tracked operator layer is the config: manifest.yaml, blueprints/, flows/,
# loops/*.yaml, specialists/*.yaml, prompts/. The wholly-runtime dirs are listed
# first; the patterns below them are runtime files that live ALONGSIDE config in
# loops/ and specialists/ — a loop's cycle state/ledger and a specialist's
# accumulated knowledge — so they are ignored by pattern, not by directory.
runs/
queue/
metrics/
loops/*.state.json
