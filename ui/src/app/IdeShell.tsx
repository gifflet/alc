// IdeShell.tsx — The IDE grid: activity bar, tool window, tabs, bottom panel, status bar.
//
// This is the desktop layout, extracted VERBATIM from Shell so that adding a
// mobile layout could not change it. Shell now only decides which one renders.
        ...
        {ui.leftCollapsed ? (
          ...
          <>
            <div style={{ width: ui.leftWidth }} className="flex shrink-0 flex-col border-r border-border">
              <div className="flex h-7 items-center justify-between border-b border-border bg-panel px-2 text-[length:var(--ui-text-label)] uppercase tracking-wide text-faint">
                <span>Project</span>
                <button
                  type="button"
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          {ui.bottomCollapsed ? (
            <button
              ...
              className="flex h-6 shrink-0 items-center gap-1.5 border-t border-border bg-panel px-3 text-[length:var(--ui-text-label)] text-faint hover:text-primary"
            >
