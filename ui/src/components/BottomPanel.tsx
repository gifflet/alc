  ...
  const body = (
    <>
      ...
      <span className="flex min-w-0 flex-wrap gap-x-2">
        ...
        {/* No shrink-0 here: it pinned the span at its content width (measured
            1780px on a 411px panel), so the message never wrapped — it scrolled.
            min-w keeps it jumping to its own full line beside a long rule name;
            grow + shrinkable is what lets the text actually wrap there. */}
        <span className="min-w-[12rem] grow text-muted">{v.message}</span>
      </span>
    </>
  )
  if (!target) {
